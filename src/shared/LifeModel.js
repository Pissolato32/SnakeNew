...
const clampFocus = (value) => Math.max(FOCUS_LEVELS.MIN, Math.min(FOCUS_LEVELS.MAX, Math.round(Number(value) || FOCUS_LEVELS.DEFAULT)));

export function normalizeFocus(input = {}) {
    const focus = {};
    for (const key of FOCUS_KEYS) {
        focus[key] = clampFocus(input[key]);
    }
    return focus;
}
...
export function createDefaultFocus() {
...
export const DEFAULT_SKILLS = Object.freeze({
    individual: { points: 0, unlocked: [] },
    family: { points: 0, unlocked: [] }
});
...
export function sameFamily(a, b) {
    return Boolean(a?.familyId && b?.familyId && a.familyId === b.familyId);
}
