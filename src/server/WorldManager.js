...
class WorldManager {
    ...
    start() {
        ...
        if (!this.regions.has('A')) this.regions.set('A', new Region('A', this.io, this.logger));
    }
    getRegion() { return this.regions.get('A'); }
    lifeListForAccount(region, token) {
        const account = this.accountLifeService.getAccount(token);
        if (!account) return [];
        return account.lifePersistentIds.map(id => Object.values(region.agentManager.getAgents()).find(a => a.persistentId === id && !a.isDead)).filter(Boolean).map(a => ({ persistentId: a.persistentId, nickname: a.nickname, familyId: a.familyId, broodId: a.broodId, generation: a.generation, controller: a.controller, isOnline: Boolean(a.isOnline), isOffline: Boolean(a.isOffline) }));
    }
    emitLifeList(socket, token) {
        const account = this.accountLifeService.getAccount(token);
        const region = this.getRegion();
        socket.emit('life-list', { lives: account ? this.lifeListForAccount(region, token) : [], account: this.accountLifeService.serialize(account) });
    }
    findOrCreateRegion(socket, strategistData) {
        const region = this.getRegion(); if (!region) return;
        ...
        if (strategistData.persistentId) {
            ...
            this.assumeLife(socket, region, selected, token); return;
        }
        const account = token ? this.accountLifeService.getOrCreateAccount(token) : null;
        ...
        if (agent && !agent.isDead && !agent.isBot) {
            if (account) agent.familyId = account.familyId;
    ...
    failLogin(socket, error) { socket.emit('login-failed', { error }); this.socketToRegionMap.delete(socket.id); }
    assumeLife(socket, region, selected, token) {
        const oldId = selected.id; delete region.agentManager.agents[oldId]; region.agentManager.agentSpatialHashing.update(selected);
    ...
    createLife(socket, data) {
        const region = this.getRegion(); const token = data?.token;
    ...
    selectLife(socket, data) {
        const token = data?.token; const region = this.getRegion();
    ...
    findAgentBySocketId(socketId) { const regionId = this.socketToRegionMap.get(socketId); const region = regionId && this.regions.get(regionId); return region ? Object.values(region.agentManager.getAgents()).find(a => a.socketId === socketId || a.id === socketId) || null : null; }
    handleDisconnect(socket) { const regionId = this.socketToRegionMap.get(socket.id); const region = regionId && this.regions.get(regionId); if (!region) return; const agent = this.findAgentBySocketId(socket.id); if (agent && !agent.isBot && !agent.isDead) { agent.isOnline = false; agent.controller = 'AI'; agent.socketId = null; agent.isOffline = true; agent.offlineSince = Date.now(); if (region.isReady) region.persistenceSystem.saveState(region.agentManager.getAgents()); } this.socketToRegionMap.delete(socket.id); }
    handleInput(socket, data) { const agent = this.findAgentBySocketId(socket.id); if (!agent || agent.controller !== 'HUMAN' || agent.isDead) return; if (Number.isFinite(data.angle)) agent.targetAngle = data.angle; if (typeof data.isBoosting === 'boolean') agent.isBoosting = data.isBoosting; if (Number.isInteger(data.seq) && data.seq > agent.lastProcessedInputSeq) agent.lastProcessedInputSeq = data.seq; }
    handleStrategyUpdate(socket, data) { const agent = this.findAgentBySocketId(socket.id); if (!agent || agent.controller !== 'HUMAN' || agent.isDead) return; agent.handleStrategyInput({ type: 'STRATEGY_UPDATE', focus: data.focus, strategy: data.strategy }); agent.strategyUpdatedAt = Date.now(); }
    tick() { const active = this.io.sockets.sockets.size > 0; if (!active) { if (!this.isSleeping) { this.isSleeping = true; for (const region of this.regions.values()) if (region.isReady) region.persistenceSystem.saveState(region.agentManager.getAgents()); } const now = Date.now(); if (now - this.lastBackgroundTick >= 5000) { const dt = Math.max(0, (now - this.lastBackgroundTick) / 1000); this.lastBackgroundTick = now; for (const region of this.regions.values()) if (region.isReady) region.simulateOfflineProgression(dt); } return; } this.isSleeping = false; this.lastBackgroundTick = Date.now(); for (const region of this.regions.values()) region.tick(); }
